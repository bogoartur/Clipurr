...
///
/// The history is stored at `~/Library/Application Support/CopyCat/history.json`.
...
struct PersistenceManager {
    ...
    /// The directory where CopyCat stores its data.
    static let directoryURL: URL = {
        let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask
        ).first!
        return appSupport.appendingPathComponent("CopyCat", isDirectory: true)
    }()
